...
/*
 * Modified buffer for the modified getch and ungetch functions as
 * we need to hold only one character now
 */
char modbuf = 0; 

/*
 * getch: Gets a (possibly pushed-back) character. Now modified to hold
 * only one character
 */
int getch(void) {
  int c;
...
/*
 * ungetch: Pushes character back in input. Now modified to hold only one
 * character
 */
void ungetch(int c) {
  if (modbuf != 0) {
    printf("ungetch: buffer already has a character\n");
  } else {
      modbuf = c;
  }
}
